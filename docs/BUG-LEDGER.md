...
| 09-04 | phase 7, the home's test | the registry keyed runs by run id, so two projects each with a `live-1` (or two walk runs both named `walk`) collided and one vanished from the list | a key that is not the identity (the folder is the run; the id is a label) -- a second owner of a fact | the registry is keyed by run dir with the id a column; an old index is rebuilt in place (`layers/registry.py`) |
| 09-04 | phase 7, the walk's gateway leg | the home's rows carried the registry's resolved `/private/var/...` and the leg compared with `/var/...` (macOS's symlink) | a path compared by two conventions | the leg compares the resolved form, the registry's (`walk.py`) |
| 09-04 | phase 7, run again from the home | the Prefect runner's probe called `asyncio.run` inside the page's running event loop, raised, reported "unavailable", and the Gateway fell back to the LocalRunner with a print nobody on the page saw; live-8-2 ran locally though Prefect was asked for | a message that hides the reason (the fallback said aloud only to a log); also a check that fails for a reason other than the one it names | coroutines run through `_run()`, a worker loop when one is already running; the handle names the runner that took the run and the Gateway keeps the fallback reason for the page (`layers/prefect_runner.py`, `layers/runner.py`, `gateway/api.py`) |
| 09-04 | live-10 / p3-implement, the container tier | ruff read `E902 No such file` on every attempt: the coder's checks staged the answer in the system temp folder, which Colima does not share, so the container saw an empty mount; the halt came after the re-ask loop, not before the run | a path the sandbox cannot reach (a new heading: a check that stages outside the run folder); also a path the walk cannot reach (the walk's own run folders lived in the system temp, so the tier was never walked on the coder) | `RunPaths.staging()`: a scratch folder inside the run, the one place every tier reaches; the coder's three staged checks use it; `available(root=)` refuses aloud when the engine cannot see the run folder; a walk under `CSMW_SANDBOX=container` keeps its runs under `~/.csmw/walk` (`state/run.py`, `layers/container_sandbox.py`, `walk.py`, the coder's `workflow.py`) |
| 09-05 | the fresh install, before going public | the plugin's bootstrap printed "ready" over a failed clone and a failed pip: `set -e` is silent inside a command group that feeds `\|\|`, so no step's failure stopped the group | an exit code that lies | every step checks its own exit and says what failed; "ready" only when `csmw` is in the venv (`csmw_coder/scripts/bootstrap.sh`) |
| 09-05 | the fresh install | pip could not install the runtime on a clean machine: Guardrails AI pins `openai<3`, PydanticAI's OpenAI extra needs `openai>=3.8`, and the dev venv only had both because they went in one at a time | a setting that works only where it was made (a second heading for: an install proven only on the author's machine) | `openai` is no longer pinned in the runtime's dependencies; it is an extra installed as a second step, and the reason is written where the pin was; the bootstrap does the two steps; the example task uses the plugin's defaults, not an API backend (`pyproject.toml`, `requirements.txt`, the coder's `bootstrap.sh` and `examples/`) |
| 09-05 | the fresh install's live run `fresh-1` | the run completed with a verdict of 0/1: pytest was only in the runtime's `checks` extra, so the fresh venv had none; `csmw doctor` had said "pytest found" because it looked on the shell's PATH, not in the interpreter the checks use | a check that reads the wrong record (the doctor) on top of a setting that works only where it was made (pytest present in the dev venv by accident) | pytest, ruff and pyright are base dependencies; one resolver (`layers.tools.resolve_binary`: beside the interpreter first, then PATH) is what the tools, the checks and the doctor all consult (`pyproject.toml`, `layers/tools.py`, `checks/pycheck.py`, `doctor.py`) |
| 09-05 | the same run | verify recorded every property "missing" and the run finished clean; the JUnit file was absent because pytest never ran (`No module named pytest`, exit 1 in 0.03s), and the check turned that into an empty result | a message that hides the reason | a missing JUnit file after the pytest tool ran is a halt carrying pytest's exit code and its last lines, never a verdict (`checks/runtests.py`) |
| 09-05 | the fresh install, second pass | on a machine whose `python3` is 3.14 the runtime could not install: Guardrails AI has no 3.14 build, `requires-python` said `>=3.11` with no ceiling, and the bootstrap took the first `python3` it saw, so the failure was a pip listing five screens long | a setting that works only where it was made (the dev machine has a 3.11 first on PATH) | `requires-python` is `>=3.11,<3.14` with the reason beside it; the bootstrap tries 3.13, 3.12, 3.11, then `python3`, takes the first in range, and names the requirement and what it found when none is (`pyproject.toml`, the coder's `bootstrap.sh`, both READMEs) |
| 09-05 | the walk, once | the gateway leg failed after 124s and the line said `ExceptionGroup: unhandled errors in a TaskGroup (1 sub-exception)`; three reruns passed in 7s, and the cause is unknown because the report swallowed it | a message that hides the reason | the walk's failure line names the leaf exceptions of a group and the cause chain (`walk._leaf_error`); the leg itself is not changed, since nothing is known to change |
...
- **a path the sandbox cannot reach** — a file a check stages, or a root a tool is handed, outside
  the folder the sandbox mounts (the run folder), so the host sees it and the container does not.
  Fix: everything a step checks lives under the run folder; the tier refuses aloud when the engine
  cannot see the run folder.
