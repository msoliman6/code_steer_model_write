...
### 7a.1 The run-status control (the bottom bar's right side)
...
the code-builder lives inside the template, which is the one thing the charter forbids ("this is
a template, not a project"). The split below moves it out without changing it. The harness
figure (docs/media/harness.svg) draws the boundary: the project band above, the template band
below, one arrow between them.

### 12.1 What stays in the template (`code_steer_model_write`)

| area | modules | why it is harness |
|---|---|---|
| shapes and ids | `spec/base.py` (Artifact, wire schema, template, guide), `spec/events.py`, `spec/task.py`, `spec/findings.py`, `spec/decisions.py`, `ids.py` | every recipe's outputs are Artifacts with code-assigned ids |
| the call | `ask.py`, `prompts.py`, `backends/*`, `providers/*`, `config.py` (settings, price table) | one schema per call, re-ask, four backends and the fake |
| the run | `driver/*`, `state/*`, `events.py`, `gates/*`, `review/rounds.py`, `worktree.py`, `artifacts/store.py`, `artifacts/render.py`, `artifacts/brief.py`, `artifacts/ledger.py` | steps from disk, one owner per fact, bounded loops, gates, brief and ledger are every recipe's stage 0 |
| code checks as a toolkit | `checks/code.py`, `checks/pycheck.py`, `checks/runtests.py`, `checks/nullimpl.py`, `checks/ownership.py` | ruff, pyright, pytest, the null run and ownership are tools any code-writing recipe calls; a recipe declares which it requires |
| recipes as plugins | `recipes/base.py` (RecipeSpec, StageSpec), `recipes/registry.py` extended to discover installed packages through the `csmw.recipes` entry-point group | the template ships no workflow of its own except the tiny debate recipe as its proof and example |
| observability | `observability/*`, `workflow/flows.py` | sinks and views |
| pages | `dashboard/*`, `settings_form.py` | the start page and the run page are built from any RecipeSpec |
...
| its figure | `docs/media/workflow*.svg` | the project's `docs/media/`, regenerated by `csmw figure code_builder` |
...
1. `dashboard/model.py::list_artifacts` maps artifact keys to stages with a hardcoded table →
   `StageSpec.artifact_keys`, declared by the recipe.
2. `settings_form.py` has `RECIPE = "code_builder"` and `build_task(recipe="code_builder")` → the
   start page takes the recipe from the registry (a dropdown when more than one is installed).
3. `dashboard/start.py` imports `registry.get("code_builder")` for the stage rail → the same.
4. `walk.py` names `code_builder` in its legs → the generic legs run on the debate recipe; recipe
   legs come from the recipe package (`RecipeSpec.walk_legs`).
5. `recipes/registry.py` → entry-point discovery, with the bundled debate recipe as the default.
6. `figure.py` is already spec-driven; `tests/test_figure.py` pins the code-builder geometry →
   the pin moves to the project, the template pins the debate figure.
...
### 12.5 The coder as a Claude Code plugin — done 2026-09-04

The coder repo is the plugin: `.claude-plugin/plugin.json` (name `csmw-coder`, three userConfig
keys: `harness_path`, `runs_dir`, `dashboard_port`), `.claude-plugin/marketplace.json` so the repo
is its own marketplace, `commands/` and `scripts/`. Installed on the user's machine as
`csmw-coder@csmw` (marketplace `csmw` = the local checkout; `claude plugin install
github:msoliman6/csmw_coder` for anyone else).

Decisions taken without the user (he asked to wake up to a plugin):

- **Three commands.** `/csmw-coder:build` writes a task from the conversation and starts the run;
  `/csmw-coder:status` prints one line per run; `/csmw-coder:dashboard` prints the page's address,
  starting it if needed. No hooks, no MCP server, no agent: the session's model only fills the
  TaskSpec, which is the contract (`plugin/task.schema.json`, generated by pydantic;
  `plugin/defaults.json` the recommended config; `plugin/task.example.json` the wording).
- **Silent by construction, and no file from the session.** The model composes the task JSON and
  hands it to `scripts/start.sh` on stdin (a session's Write outside its project needs a permission
  prompt; Bash on a bundled script does not). The script saves it in the plugin's data dir,
  validates it, starts `csmw run` under nohup with its output in the run dir, and prints exactly one line; the command tells the model to answer
  with that line verbatim and never to poll, read the run dir or report. The report is the hand-back
  (`status` names its path when it exists); telling the session is still later.
- **Runs live in `~/.csmw/runs`**, outside any project, so a session's repo stays clean.
- **The environment lives in the plugin's data dir**: `scripts/bootstrap.sh` makes a venv there
  with the harness (from `harness_path`, a known checkout, or `gh repo clone` of the private repo)
  and the plugin installed; it reuses a checkout's venv when one exists so the first build is
  instant. `CSMW_CLI_USE_LOGIN=1` is the default: both sides run on the user's CLI logins.
