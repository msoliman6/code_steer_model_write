...
### 12.5 Later: the coder as a Claude Code plugin (sketch, not now)

The project repo also carries `.claude-plugin/` so a Claude Code session can install it. A
skill (`/coder`) takes the session's context and writes a `task.json` (a TaskSpec: the brief, the
recommended config as defaults, the mode) into the project's runs dir, then starts the run
detached (`csmw resume`, the same path the start page uses) and returns at once, printing one
line: the run id and the dashboard URL. The run never writes to the session. The dashboard is
where it is watched. When it finishes, a report file is the hand-back; whether the session is
told is a later decision. The TaskSpec's JSON schema, already generated by pydantic, is the
contract between the session and the run.
