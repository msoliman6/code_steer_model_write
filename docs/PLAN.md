...
- **Silent by construction.** `scripts/start.sh` validates the task, starts `csmw run` under nohup
  with its output in the run dir, and prints exactly one line; the command tells the model to answer
